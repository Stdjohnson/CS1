...
-- And here it becomes absolutely clear that all we're doing here is
...
set_cardinality (mkSet l) = list.foldr add O (map count1 l)
...
combine nil l = l
combine l nil = l
combine (h1::t1) (h2::t2) = ite (eql h1 h2)
                                (h1::(combine t1 t2))
                                (h1::h2::(combine t1 t2))
{-my combine function does not retain the oder
of the combined lists because
...
set_forall p (mkSet (h::t)) = ite (p h)
                                  (set_forall p (mkSet t))
                                  (false)
-------------------------------------
...
pwer: list a -> (list (list a))
pwer nil = nil
pwer (h::t) = (h::nil)::(pwer t)

--powr: list a -> (list (list a))
--powr nil = nil
-------------------------------------
